...
const INTENT_FALLBACK_TEXT =
  'Choose only a registered Mac Control action and supplied target. Generic dictation or text entry belongs to the separate Dictation shortcut and is unsupported. Return JSON {"intent":"action|clarify|unsupported","actionKind":"...","targetId":"...","parameters":{}}. For clarify and unsupported, return only the intent and omit actionKind, targetId, and parameters. For scroll parameters use only direction up|down and amount 1-100; for focus use only role and label; for select and click use only label; for press use only ArrowUp, ArrowDown, ArrowLeft, ArrowRight, PageUp, PageDown, Home, End, Tab, Escape, Enter, A, C, or V with optional Shift or Command. Do not return parameters for other actions. Never include confidence, permissions, shell commands, URLs, file IDs, email content, or generic typed text.';
