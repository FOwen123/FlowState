...
export type ParsedIntentFallback = ParsedFallbackDecision;
...
const INTENT_FALLBACK_TEXT =
  'Choose only a registered action and supplied target. Return JSON {"intent":"dictation|action|clarify|unsupported","actionKind":"...","targetId":"...","parameters":{}}. For dictation, clarify, and unsupported, return only the intent and omit actionKind, targetId, and parameters. For scroll parameters use only direction up|down and amount 1-100; for focus use only role and label; for select use only label; for press use only ArrowUp, ArrowDown, ArrowLeft, ArrowRight, PageUp, PageDown, Home, End, Tab, Escape, Enter, A, C, or V with optional Shift or Command. Do not return parameters for other actions. Never include confidence, permissions, shell commands, URLs, file IDs, email content, or rewritten dictation text.';
